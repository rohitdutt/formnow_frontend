...
const CreateForm = () => {
    ...
    return (
        <div className={"h-auto bg-gray-200 relative"}>
            <div className={"py-5 px-10 flex justify-between items-center bg-gray-200"}>
                <p className={"text-2xl font-semibold text-gray-600"}>{organization.name}</p>
                <div className={"flex items-center"}>
                    Select user : <SelectEmployee setSelected={setSelected} selected={selected} employees={organization.employees}/>
                </div>
            </div>
            <div className={"flex flex-col items-center"}>
                <form className={"bg-white w-5/6 md:w-4/6 py-2 rounded-lg shadow-2xl"}>
                    <p className={"text-xl text-center font-semibold text-gray-600"}>Create your form</p>
                    <div className={"flex flex-row justify-center space-x-4 items-center px-4 sm:px-10 py-2"}>
                        {/* <label className="text-gray-700 dark:text-gray-200" for="username"></label> */}
                        <p className={"text-lg font-semibold text-gray-600"}>Enter title for your form :</p>
                    ...
                    <div className={"flex flex-row justify-center space-x-4 items-center px-4 sm:px-10 py-2"}>
                        <p className={"text-lg font-semibold text-gray-600"}>Enter description for form : </p>
                        <textarea id="description" type="text" className="block w-full md:w-3/6 px-4 py-1 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-purple-500 dark:focus:border-blue-500 focus:outline-none focus:ring focus:ring-purple-400" onChange={ e => setDescription(e.target.value)}/>
                    </div>
                </form>
                {
                    formFields.map((field)=>(
                        <FormFieldCard key={field.id} field={field} setFormFields={setFormFields} formFields={formFields} selectedType={selectedType} setSelectedType={setSelectedType}/>
                    ))
                }
